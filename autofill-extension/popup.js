...
const modelUsedBadge = document.getElementById('modelUsed');

function showModal(text, model) {
    coverLetterTextArea.value = text;
    modelUsedBadge.textContent = `Generated with ${model}`;
...
document.getElementById('exportMdBtn').addEventListener('click', () => {
    const text = coverLetterTextArea.value;
    const markdown = `# Cover Letter\n\n${text}\n\n---\n*Generated by JobAutoFill Magic*`;
...
document.getElementById('coverletter-btn').addEventListener('click', async () => {
    const statusEl = document.getElementById('status');
    const selectedModel = document.getElementById('model-select').value;

    // Prompt user to paste vacancy description
    const vacancyText = prompt('Paste the job vacancy description here:');
    if (!vacancyText || vacancyText.trim().length < 100) {
        statusEl.textContent = 'Vacancy text is too short';
        statusEl.style.color = '#ef4444';
        return;
    }

    statusEl.textContent = `Generating with ${selectedModel}...`;
    statusEl.style.color = 'var(--accent)';
    ...
    try {
        ...
        if (data.coverLetter) {
            ...
            const coverLetterWithWatermark = data.coverLetter + watermark;

            statusEl.textContent = 'Cover letter generated!';
            statusEl.style.color = '#10b981';
            showModal(coverLetterWithWatermark, selectedModel);
        } else {
